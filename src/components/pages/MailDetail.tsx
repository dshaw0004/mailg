...
import { useMail, type EmailDetail, type Attachment } from "@/context/MailContext";
...
export default function MailDetail({ id }: MailDetailProps) {
        ...
        <div className="h-full flex flex-col bg-[#0a0a0a] animate-fade-in relative z-0">
            <div className="px-6 py-3 flex items-center justify-between border-b border-white/[0.04] sticky top-0 bg-[#0a0a0a]/80 backdrop-blur z-10">
                ...
                <div className="flex items-center gap-1">
                    ...
                    />
                    <ToolbarButton icon={Clock} tooltip="Snooze" />
                </div>
            </div>
                ...
                <div className="max-w-3xl mx-auto">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <div className="flex gap-2">
                            {email.labels.map((label) => (
                                <span
                                    key={label}
                                    className="px-2 py-1 bg-white/5 border border-white/10 rounded text-xs text-neutral-400 font-medium"
                                >
                                    {label}
                                </span>
                            ))}
                    ...
                    <div className="flex items-start justify-between mb-8 group">
                        ...
                        <div className="flex items-center gap-4 text-sm text-neutral-400">
                            <span className="text-xs group-hover:text-neutral-300 transition-colors">
                    ...
                    <div
                        className="text-neutral-300 text-sm leading-relaxed whitespace-pre-wrap font-sans"
                    ...
                    <div className="mt-12 p-4 border border-white/[0.08] rounded-xl bg-neutral-900/30 flex gap-4 text-neutral-500 text-sm cursor-text hover:bg-neutral-900/50 hover:border-white/20 transition-all duration-300">
                        <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 flex items-center justify-center text-[10px] text-white font-medium flex-shrink-0">
                            JD
                        </div>
...
function AttachmentCard({ attachment, emailId }: AttachmentCardProps) {
    ...
    return (
        <div
            className="flex items-center gap-3 p-3 bg-white/[0.02] border border-white/[0.06] rounded-lg hover:bg-white/[0.04] transition-colors cursor-pointer"
